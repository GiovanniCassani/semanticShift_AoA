...
# get only the top half of a correlation matrix
get_upper_tri <- function(cormat){
  cormat[lower.tri(cormat)]<- NA
  return(cormat)
}

# find the best lambda coefficient to transform the input variable, considering the interval
# defined by a min and max value, considering increments of 0.1
...
##### Transformations ####
# relevel the dominant pos tag using nouns as referents (coefficients of continuous predictors 
# don't change when using other ways of coding the categorical variable, such as weighted effects
# coding)
df$Dom_Pos = relevel(factor(df$Dom_Pos), 'Noun')

# box-cox transform and z-transform all continuous predictors
...
cormat <- round(cor(df[c("AoA", "C.z", "CD.z", "F.z", "L.z", "FpM.z", "OLD.z", 
                         "LNC.z", "VC.z", "J.z", "rLNC.z", "rVC.z", "rJ.z")]),3)
...
##### single vars #####
summary(lm(AoA ~ J.z, data = df))
summary(lm(AoA ~ VC.z, data = df))
summary(lm(AoA ~ LNC.z, data = df))
...
##### d=40, w=20, minCount=10 #####
# repeat for the measures computed using a window size of 20 words to the left and right
df = read.csv("data/processed/LNC.fixed.minCount10/semanticShift_d40_w20.csv", 
              header = T, sep = ',')  
df = df %>% 
  dplyr::rename(
    FpM = FpM.SUM,
...
ggplot(data = melted_cormat, aes(Var2, Var1, fill = value))+
  ...
  coord_fixed() + 
  geom_text(aes(Var2, Var1, label = value), color = "white", size = 4) +
  ggtitle("Pairwise correlations (d=40, w=20, minCount=10)") +
